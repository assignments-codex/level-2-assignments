...
- At least **one API request to a chatbot** with basic **error handling** (friendly message on failure).
- **Safe DOM updates** (`textContent` or equivalent).
- **README** with setup notes, links, and a short architecture description.

> Note: a **chat interface** using an AI provider (e.g., Google Gemini, Cloudflare Workers AI) is required for this capstone.

---

## Friday presentation (10-12 minutes total)

1. **Live site demo** (about 2 min): problem -> solution -> core flow.
2. **Code walkthrough** (about 3-4 min): show specific files you're proud of; explain decisions and trade-offs.
3. **Q&A on code** (about 2-3 min): be ready to explain any part of your repo.
4. **Live coding** (about 3-4 min, no AI): instructor will choose a small task related to Level-2 skills.
